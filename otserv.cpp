...
bool argumentsHandler(StringVec args)
{
	...
	for(StringVec::iterator it = args.begin(); it != args.end(); ++it)
	{
		...
		if((*it) == "--version" || (*it) == "-v")
		{
			...
			"Compiled with " << BOOST_COMPILER << " for architecture "
...
ServiceManager* services)
{
	...
	std::clog << "The " << SOFTWARE_NAME << " Version: (" << SOFTWARE_VERSION << "." << MINOR_VERSION << "." << PATCH_VERSION << REVISION_VERSION << ") - Codename: (" << SOFTWARE_CODENAME << ")" << std::endl
		...
		<< std::endl << "A server developed by: " SOFTWARE_DEVELOPERS "." << std::endl;
	std::stringstream ss;
#ifdef __DEBUG__
	ss << " GLOBAL";
#endif
	...
	if(GetLastError() == ERROR_ALREADY_EXISTS)
		startupErrorMessage("Another instance of the Collapser Server is already running with the same worldId.\nIf you want to run multiple servers, please change the worldId in configuration file.");

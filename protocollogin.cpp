...
void ProtocolLogin::onRecvFirstMessage(NetworkMessage& msg)
{
	...
	Account account;
	if(!IOLoginData::getInstance()->loadAccount(account, name) || (account.name != "0" && !encryptTest(account.salt + password, account.password)))
	{
	...
	if(account.name != "0" && !g_config.getBool(ConfigManager::ACCOUNT_MANAGER) && !account.charList.size())
	{
	...
	IOLoginData::getInstance()->removePremium(account);
	if(account.name != "0" && !g_config.getBool(ConfigManager::ACCOUNT_MANAGER) && !charList.size())
	{
